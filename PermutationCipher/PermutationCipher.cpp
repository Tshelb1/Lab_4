...
inline int PermutationCipher::getValidKey(const int k, const std::wstring & s)
{
    if (k<=0)
        ...
        throw cipher_error("Предупреждение! Ключ длиннее, чем длина текста");
    else
        return k;
}
inline std::wstring PermutationCipher::getValidOpenText(const std::wstring & s)
{
    ...
    if (tmp.empty())
        throw cipher_error("Ошибка! Отсутствует входной текст!");
    return tmp;
}
...
inline std::wstring PermutationCipher::getValidCipherText(const std::wstring & s)
{
    ...
    if (tmp.empty())
        throw cipher_error("Ошибка! Отсутствует выходной текст!");
    return tmp;
}