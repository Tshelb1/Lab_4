...
*  @warning Реализация  русскоязычных сообщений.
*/
...
/// @brief  Алфавит для текста,  использующийся в данной программе
     ...
     * @details В векторе типа "int" с именем "result" формируются числа. Числа являются индексами алфавита "numAlpha" из строки, которая выдается на вход.
     * @code
     *  vector<int> result;
     *   for(auto c:s) {
    ...
    /** @brief  Контролируемость ключа
     * @details Ключ проверяется на пустоту. Если проверка закончилась успешно, то
     * ключ проверяется на наличие недопустимых символов. 
     * @warning Строчные буквы алфавита переводятся в прописные.
     * @param wstring s - ключ, который нужно проверить на наличие ошибок, в виде строк.
    ...
    /** @brief  Контролируемость текста при шифровании или расшифровании
     *  @details Сначала текст проверяется на пустоту . Если проверка закончилась успешно, то
...
public:
    ...
    /** @brief  Конструктор для ключа
      * @details Цикл "for" построен по строке-алфавиту и на каждом выполненном шаге добавляет в 
      *   ассоциативный массив символ и его номер.
      * @code
     ...
     *   алфавита. Если ключ меньше/короче текста, то при индексации ключа выполняется операция
     ...
     * @endcode
     *  Если при зашифровывании прибавляется значение ключа, то при расшифровывании значения ключа вычитается. Для того 
     *  чтобы не получить отрицательных значений, выполняется еще прибавление значения модуля, так как 
     *  такое прибавление не влияет на результат модуля.
     *  @code
     *   for(unsigned i=0; i < work.size(); i++) {
